  ...
  return (
    <div className="flex items-center gap-1">
      {[1, 2, 3, 4, 5].map((i) => (
        <Star
          key={i}
          className={`h-5 w-5 ${i <= Math.round(rating) ? 'fill-amber-400 text-amber-400' : 'fill-slate-200 text-slate-200'}`}
        />
      ))}
    </div>
  );
              ...
              <div className="p-6 sm:p-8">
                <div className="flex flex-col sm:flex-row gap-6">
                  ...
                  <div className="relative flex-shrink-0">
                    <div className={`h-32 w-32 rounded-2xl overflow-hidden flex items-center justify-center text-4xl font-bold shadow-sm border-4 border-slate-50 ${doctor.avatarBg}`}>
                      {doctor.img ? (
                    ...
                    {doctor.badge && (
                      <span className={`absolute -bottom-2 -right-2 text-xs px-2.5 py-1 rounded-full font-semibold border-2 border-white ${doctor.badgeColor}`}>
                        {doctor.badge}
                      </span>
                    )}
                  </div>
                  ...
                  <div className="flex-1">
                    <div className="flex items-start justify-between flex-wrap gap-2 mb-2">
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          {doctor.verified && (
                            <span className="flex items-center gap-1 bg-blue-50 text-blue-700 text-xs font-semibold px-2 py-0.5 rounded-full border border-blue-200">
                              <CheckCircle2 className="h-3.5 w-3.5" /> Verified
                            </span>
                          )}
                        </div>
                        <p className="text-teal-700 font-semibold text-lg">{doctor.title}</p>
                        <p className="text-slate-500 mt-0.5">{doctor.specialty} · {doctor.experience} years experience</p>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      <span className="bg-slate-100 text-slate-600 text-xs font-semibold px-3 py-1 rounded-full">
                        QAR {doctor.fee} consultation fee
                      </span>
                    </div>
                  </div>
                </div>

                {/* Location & Insurance row */}
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 sm:p-8">
              ...
              <div className="rounded-xl overflow-hidden border border-slate-200">
                <div className="bg-slate-100 h-48 flex items-center justify-center">
                  <div className="text-center">
                    <MapPin className="h-10 w-10 text-teal-500 mx-auto mb-2" />
                    <p className="font-semibold text-slate-700">{doctor.clinic}</p>
                    <p className="text-slate-500 text-sm">{doctor.address}, {doctor.city}, Qatar</p>
                  </div>
                </div>
              </div>
              <p className="text-slate-500 text-sm mt-3">
                <MapPin className="h-4 w-4 inline text-slate-400 mr-1" />
                {doctor.address}, {doctor.city}, Qatar · {doctor.distance} km from your location
              </p>
            </div>
          </div>
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm sticky top-24 overflow-hidden">
              ...
              <div className="p-6">
                ...
                <div className="space-y-2 mb-4">
                  {doctor.availableSlots.slice(0, 4).map((slot, i) => (
                    <button
                      ...
                      className="w-full py-3 px-4 rounded-lg border border-slate-200 bg-slate-50 text-slate-700 text-sm font-medium hover:bg-teal-50 hover:border-teal-200 hover:text-teal-800 transition-colors flex items-center gap-2"
                    >
                      <Clock className="h-4 w-4 text-slate-400" /> {slot}
                    </button>
                  ))}
                </div>
                ...
                <button
                  onClick={() => navigate(`/book/${doctor.id}`)}
                  className="w-full bg-teal-600 hover:bg-teal-700 text-white font-bold py-3.5 px-4 rounded-xl shadow-sm transition-colors text-base"
                >
                  Book Appointment
                </button>

                <p className="text-center text-xs text-slate-400 mt-3">Free cancellation up to 24h before</p>
                ...
                <div className="mt-4 pt-4 border-t border-slate-100">
                  ...
                  <div className="flex items-center justify-between text-sm mt-2">
                    ...
                    <span className={`font-semibold ${doctor.insuranceStatus === 'in' ? 'text-green-600' : 'text-slate-600'}`}>
              ...
              <div className="space-y-2">
                ...
                {doctor.verified && (
                  <div className="flex items-center gap-2 text-sm text-slate-600">
                    ...
                    <span>Tabiby Verified Provider</span>
                  </div>
                )}
                <div className="flex items-center gap-2 text-sm text-slate-600">
